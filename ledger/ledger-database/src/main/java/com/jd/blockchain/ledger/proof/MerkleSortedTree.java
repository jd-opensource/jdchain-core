...
public class MerkleSortedTree<T> implements Transactional {
	...
	public static final int TREE_DEGREE = 8;
...
//		return new MerkleProofImpl(sn, nodePath);
	 ...
	 * @return
	 */
	public DataNode setData(long sn, T data) {
		if (sn < 0) {
	...
	private DataNode createDataNode(T data) {
		//TODO: create data node;
	...
	private static class RehashTask extends RecursiveTask<List<PathNode>> {
		...
		public RehashTask(PathNode[] pathNodes, MerkleSortedTree tree, ExPolicyKVStorage kvStorage) {
			this.tree = tree;
	...
	private DataNode loadDataNode(HashDigest hashBytes, boolean verify) {
		...
		DataNode dataNode = decodeDataNode(bytes);
	...
	private DataNode decodeDataNode(byte[] dataNodeBytes) {
		//TODO: create data node;
		throw new IllegalStateException("Not implemented!");
	}
