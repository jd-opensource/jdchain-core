    ...
    protected void doInit() {
        ...
        this.bootstrap.childHandler(new ChannelInitializer<SocketChannel>() {
            ...
            protected void initChannel(SocketChannel channel) {
                ...
                if (sslEnable) {
                    //重写引擎实现方法,
                    String protocol = System.getProperty("bolt.ssl.protocol");
                    SSLEngine engine = null;

                    if (GmSSLProvider.isGMSSL(protocol)) {
                        SSLContext sslContext = initSSLContext(protocol);
                        ...
                        engine = initSSLContext().newEngine(channel.alloc());
                    }
...
//            tmf.init(ks);
    ...
    private SSLContext initSSLContext(String protocol) {
        InputStream in = null;
        try {
            String keyStoreType = option(BoltServerOption.SRV_SSL_KEYSTORE_TYPE);
            if (keyStoreType == null) {
            ...
            sslContext.init(kmf.getKeyManagers(), tmf == null ? new TrustManager[]{new X509TrustManager() {
